...
**Feature Design & Project Structure**
- When designing a new feature, refer to `docs/architecture/structure.md` for folder organization guidelines.
- Organize feature-specific code in `src/app/features/<feature-name>/` folder.
- Consider moving shared classes/models to `shared/` or `core/` folders when used across multiple features.
...
- **One attribute per line**: Each HTML attribute should be on its own line for better readability and maintainability
- **Consistent indentation**: Use 2-space indentation for attributes
- **Logical attribute ordering**:
  1. Structural attributes (`type`, `id`, `name`)
  2. Static classes (`class`)
  3. Dynamic class bindings (`[class.x]`, `[ngClass]`)
  4. Style bindings (`[style.x]`, `[ngStyle]`)
  5. Property bindings (`[property]`)
  6. Event bindings (`(event)`)
  7. Two-way bindings (`[(ngModel)]`)
  8. Template reference variables (`#ref`)
  9. Structural directives (`*ngIf`, `*ngFor`) - should be on separate elements
  10. ARIA and accessibility attributes (`aria-label`, `role`)

**Good Example**:
```html
<!-- ✅ GOOD: One attribute per line, well organized -->
<button
  type="button"
  class="btn"
  [class.btn-primary]="confirmation()?.confirmButtonClass === 'primary'"
  [class.btn-danger]="confirmation()?.confirmButtonClass === 'danger'"
  [class.btn-warning]="confirmation()?.confirmButtonClass === 'warning'"
  [disabled]="isLoading()"
  (click)="onConfirm()"
...
**Bad Example**:
```html
<!-- ❌ BAD: Multiple attributes on same line, hard to read -->
<button type="button" class="btn" [class.btn-primary]="confirmation()?.confirmButtonClass === 'primary'"
  [class.btn-danger]="confirmation()?.confirmButtonClass === 'danger'" [disabled]="isLoading()"
  (click)="onConfirm()" aria-label="Confirm action">
  {{ confirmation()?.confirmText }}
</button>
```

**Benefits of one-attribute-per-line**:
- Easier code review (clear git diffs)
...
- **Centralize Constants**: Extract all magic numbers and strings to dedicated constant files (e.g., `task.constants.ts`)
  - Use descriptive constant names (MAX_TASK_DEPTH, DESCRIPTION_TRUNCATE_LENGTH)
  - Group related constants into objects (PAGINATION, TREE_VIEW)
  - Export constants from index files for easy importing

- **Avoid Anti-patterns**:
  - Never use `setTimeout` for waiting on async operations; use reactive RxJS operators (skip, filter, take)
  - Replace browser native `confirm()` with custom confirmation services for better UX
  - Avoid direct DOM manipulation; use Angular's templating and binding

- **Reusable Services**: Create service-based patterns for common UI needs
  - ConfirmationService for user confirmations (with Observable API)
...
- **Type Safety Beyond Basic Types**:
  - Define configuration interfaces (ConfirmationConfig, NotificationConfig)
  - Use discriminated unions for state types
  - Avoid `as any` even in edge cases; prefer type guards or unknown
